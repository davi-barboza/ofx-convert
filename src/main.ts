...
export class Ofx {
  ...
  getHeaders(): OFXMetaData {
    ...
    return convertMetaDataToObject(validatedMetaDataList, !!this.config.nativeTypes) as OFXMetaData;
  }
  ...
  private sanitizeValue(field: string, value: string) {
    ...
    } else if (this.config.nativeTypes && isValidNumberToConvert(field, fieldValue)) {
      return `${fieldValue},`;
    }
    return `"${fieldValue}",`;
  }
  ...
  private sanitize(line: string) {
    ...
    const replacer = (matched: string) => sanitizeValue.call(this, field, matched);
    if (line.match(/{(\w|\W)+/)) {
  ...
  private configFinancialInstitutionTransactionId(fitString: string) {
    ...
    if (fitId === 'separated') return extractFinancialInstitutionTransactionId(fitString);
    return `"${fitString}",`;
  }
  ...
  getTransactionsSummary() {
    ...
    const { DTEND, DTSTART, STRTTRN } = jsonData.OFX.BANKMSGSRSV1.STMTTRNRS.STMTRS.BANKTRANLIST;
