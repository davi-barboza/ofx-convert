...
function separatePartsOfDate(date: string) {
  ...
  const [offset, timezone] = date.slice(14).replace('[', '').replace(']', '').split(':');
  return {
    yyyy: year,
    yy: year.slice(2),
    y: year,
...
export function getBankTransferListText(ofxContent: string) {
  ...
  const endIndex = ofxContent.lastIndexOf(END_TEXT_BANK_TRANSFER) + END_TEXT_BANK_TRANSFER.length;
  ...
  const newListText = `"STRTTRN":[${oldListText.replace(startRgx, '{').replace(endRgx, '},')}]`;
  return { oldListText, newListText };
}

export function convertMetaDataToObject(stringList: string[], nativeTypes: boolean) {
  const result: { [key: string]: any } = {};
  for (const line of stringList) {
    const [key, value] = line.split(':');
    const sanitizedKey = key.replace('\n', '');
    result[sanitizedKey] = nativeTypes && isValidNumberToConvert(key, value) ? Number(value) : value;
  }
  return result;
}
